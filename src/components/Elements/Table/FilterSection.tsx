...
const FilterItem = <T,>({
  ...
  const options = column.filterOptions || filterOptions[columnId] || [];
  const isLoading = filterLoading[columnId] || false;
  const isFeeCodeColumn = columnId.toLowerCase().includes("fee_code");
    ...
    <>
      ...
      >
        <Autocomplete
          ...
          onInputChange={(event, newInputValue, reason) => {
            if (event && reason === "input" && !column.filterOptions) {
              const valueToUse = isFeeCodeColumn
                ? newInputValue.toUpperCase()
                : newInputValue;
              handleSearchChange(column.id, valueToUse);
            }
          }}
          renderInput={(params) => (
            <TextField
          ...
          key={`autocomplete-${columnId}-${options.join("-")}`} // Force re-render on options change
        />
      </StyledFormControl>
    </>
  );
...
const FilterSection = <T,>({
  ...
  return (
    <FiltersContainer>
      {columns
        .filter((col: TableColumn<T>) => col.filterable)
